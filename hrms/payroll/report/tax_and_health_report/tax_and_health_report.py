...
# 		_("CID") + "::120", 
# 		_("TPN#") + "::80",
# 		_("Basic Salary") + ":Float:120", 
# 		_("Allowances") + ":Float:120", 
# 		_("Arrears") + ":Float:120",
# 		_("Gross Salary(A)") + ":Float:120", 
# 		_("PF Amount(B)") + ":Float:120", 
# 		_("GIS Amount(C)") + ":Float:120",
# 		_("Net Salary(A-(B+C))") + ":Float:140", 
# 		_("Salary Tax(X)") + ":Float:120", 
# 		_("Health Contr(Y)") + ":Float:120",
# 		_("Total(X+Y)") + ":Float:120", 
...
def get_data(filters):
	...
	query = f"""
		SELECT
			...
			SUM(CASE WHEN t2.salary_component = 'Basic Pay' THEN IFNULL(t2.amount, 0) ELSE 0 END) AS basicpay,
			SUM(CASE
					WHEN t2.parentfield = 'earnings' THEN
						CASE
							WHEN t2.salary_component = 'Basic Pay' THEN 0
							WHEN t2.salary_component = 'Salary  Arrears' THEN 0
							ELSE IFNULL(t2.amount, 0)
						END
					ELSE 0
				END) AS allowances,
			SUM(CASE WHEN t2.salary_component = 'Salary  Arrears' THEN IFNULL(t2.amount, 0) ELSE 0 END) AS arrears,
		...
		WHERE
			t1.docstatus = 1 {conditions}
		GROUP BY
			t1.employee, t3.employee_name, t1.designation, t3.passport_number,
			t3.tpn_number, t1.company, t1.cost_center, t1.branch, t1.department,
			t1.division, t1.section, t1.fiscal_year, t1.month
	"""
...
def get_conditions(filters):
	conditions = ""
	if filters.get("month"):
		month_index = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"].index(filters["month"]) + 1
		filters["month"] = month_index
		conditions += " AND t1.month = %(month)s"
	...
	if filters.get("cost_center"):
		conditions += """ AND EXISTS (
			...
			WHERE t1.cost_center = cc.name AND (cc.parent_cost_center = %(cost_center)s OR cc.name = %(cost_center)s)
		)"""
	return conditions, filters
