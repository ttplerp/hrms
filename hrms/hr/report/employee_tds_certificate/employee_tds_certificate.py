...
def get_data( filters=None):
	...
	r.receipt_number, DATE_FORMAT(r.receipt_date, '%d-%m-%Y') AS receipt_date
	 from `tabSalary Slip` a, `tabTDS Receipt Entry` r, `tabTDS Receipt Update` tds, `tabTDS Reeipt Update Employees` tdre
	 where tds.name = r.tds_receipt_update and tds.name = tdre.parent and a.employee = tdre.employee and a.fiscal_year = r.fiscal_year and a.month = r.month and a.docstatus = 1 and r.purpose = 'Employee Salary' and a.fiscal_year = """ + str(filters.fiscal_year)+"""
