...
def get_data(filters,designations):
 ...
 e.cost_center,
	...
	# Get local workers count from Muster Roll Attendance for Dashboard
	lw_data = frappe.db.sql('''
		SELECT 
			cost_center, 
			SUM(local_workers) as total_lw
		FROM 
			`tabMuster Roll Attendance for Dashboard` 
		WHERE 
	...
	# Add local workers to total and present count
	for lw in lw_data:
		cost_center = lw['cost_center']
		if cost_center not in result:
			result[cost_center] = {"cost_center": cost_center}
		
		# Add to total
		if 'total' not in result[cost_center]:
			result[cost_center]['total'] = 0
		result[cost_center]['total'] += lw['total_lw']
		
		# Add to present (since local workers are always considered present)
		if 'present' not in result[cost_center]:
			result[cost_center]['present'] = 0
		result[cost_center]['present'] += lw['total_lw']
	...
	for i in regular_emp_att:
		if i.cost_center in result:
			...
			elif i.status in ['Present','On Tour']:
	...
	for i in total_emp:
		if i.cost_center in result:
			if 'total' not in result[i.cost_center]:
				result[i.cost_center]['total'] = 0
