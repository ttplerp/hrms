...
def get_columns(filters: Filters) -> List[Dict]:
	...
	columns.extend(
		[
			...
			{"label": _("MR Employee Name"), "fieldname": "mr_employee_name", "fieldtype": "Data", "width": 120},
		]
	)
...
def get_data(filters: Filters, attendance_map: Dict) -> List[Dict]:
	mr_employee_details, group_by_param_values = get_mr_employee_related_details(
		filters.group_by, filters.company, filters.branch
	)
	...
	if filters.group_by:
		...
		for value in group_by_param_values:
			...
			records = get_rows(mr_employee_details[value], filters, holiday_map, attendance_map)

			if records:
				data.append({group_by_column: frappe.bold(value)})
				data.extend(records)
	else:
		data = get_rows(mr_employee_details, filters, holiday_map, attendance_map)
...
def get_attendance_map(filters: Filters) -> Dict:
	...
	query = (
		...
		.select(
			Attendance.mr_employee,
			Extract("day", Attendance.date).as_("day_of_month"),
			Attendance.status,
			Attendance.shift,
		)
		.where(
			...
			& (Extract("month", Attendance.date) == filters.month)
			& (Extract("year", Attendance.date) == filters.year)
		)
	)
	if filters.mr_employee:
		query = query.where(Attendance.mr_employee == filters.mr_employee)
	query = query.orderby(Attendance.mr_employee, Attendance.date)

	attendance_list = query.run(as_dict=1)
	attendance_map = {}

	for d in attendance_list:
		attendance_map.setdefault(d.mr_employee, frappe._dict()).setdefault(d.shift, frappe._dict())
		attendance_map[d.mr_employee][d.shift][d.day_of_month] = d.status

	return attendance_map


def get_mr_employee_related_details(group_by: str, company: str, branch: str) -> Tuple[Dict, List]:
	"""Returns
	...
	query = (
		...
		.select(
			Employee.name,
			Employee.person_name.as_("mr_employee_name"),
			Employee.branch,
			Employee.company,
	...
	mr_employee_details = query.run(as_dict=True)

	group_by_param_values = []
	emp_map = {}

	if group_by:
		for parameter, mr_employees in groupby(mr_employee_details, key=lambda d: d[group_by]):
		...
		for emp in mr_employee_details:
...
def get_rows(
	mr_employee_details: Dict, filters: Filters, holiday_map: Dict, attendance_map: Dict
) -> List[Dict]:
	records = []
	default_holiday_list = frappe.get_cached_value("Company", filters.company, "default_holiday_list")

	for mr_employee, details in mr_employee_details.items():
		emp_holiday_list = details.holiday_list or default_holiday_list
		holidays = holiday_map.get(emp_holiday_list)

		if filters.summarized_view:
			attendance = get_attendance_status_for_summarized_view(mr_employee, filters, holidays)
			if not attendance:
				continue

			leave_summary = get_leave_summary(mr_employee, filters)
			entry_exits_summary = get_entry_exits_summary(mr_employee, filters)

			row = {"mr_employee": mr_employee, "employee_name": details.employee_name}
			...
			mr_employee_attendance = attendance_map.get(mr_employee)
			if not mr_employee_attendance:
				continue

			attendance_for_mr_employee = get_attendance_status_for_detailed_view(
				mr_employee, filters, mr_employee_attendance, holidays
			)
			# set employee details in the first row
			attendance_for_mr_employee[0].update(
				{"mr_employee": mr_employee, "mr_employee_name": details.mr_employee_name}
			)

			records.extend(attendance_for_mr_employee)
...
def get_attendance_status_for_summarized_view(
	mr_employee: str, filters: Filters, holidays: List
) -> Dict:
	...
	"""
	summary, attendance_days = get_attendance_summary_and_days(mr_employee, filters)
...
def get_attendance_summary_and_days(mr_employee: str, filters: Filters) -> Tuple[Dict, List]:
	Attendance = frappe.qb.DocType("Muster Roll Attendance")
	...
	summary = (
		...
		.where(
			(Attendance.docstatus == 1)
			& (Attendance.mr_employee == mr_employee)
			& (Attendance.company == filters.company)
			& (Extract("month", Attendance.date) == filters.month)
			& (Extract("year", Attendance.date) == filters.year)
		)
	...
	days = (
		frappe.qb.from_(Attendance)
		.select(Extract("day", Attendance.date).as_("day_of_month"))
		.distinct()
		.where(
			(Attendance.docstatus == 1)
			& (Attendance.mr_employee == mr_employee)
...
def get_attendance_status_for_detailed_view(
	mr_employee: str, filters: Filters, mr_employee_attendance: Dict, holidays: List
) -> List[Dict]:
	...
	for shift, status_dict in mr_employee_attendance.items():
		row = {"shift": shift}
...
def get_leave_summary(mr_employee: str, filters: Filters) -> Dict[str, float]:
...
def get_entry_exits_summary(mr_employee: str, filters: Filters) -> Dict[str, float]:
...
def get_attendance_years() -> str:
	...
	year_list = (
		frappe.qb.from_(Attendance)
		.select(Extract("year", Attendance.date).as_("year"))
		.where(Attendance.docstatus != 0 or 2)
		.distinct()
	).run(as_dict=True)
...
def get_chart_data(attendance_map: Dict, filters: Filters) -> Dict:
	...
	for day in days:
		...
		for mr_employee, attendance_dict in attendance_map.items():
