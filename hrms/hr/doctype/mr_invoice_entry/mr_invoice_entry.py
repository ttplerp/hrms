...
class MRInvoiceEntry(Document):
    ...
    def calculate_totals(self):
        ...
        # Update main totals
        self.total_grand_total = flt(total_grand_total, 2)
        self.total_salary_tax =round(get_salary_tax(flt(total_grand_total) - (flt(total_grand_total) * 0.15)),)

        self.total_net_payable_amount = flt(flt(total_grand_total) - flt(self.total_salary_tax), 2)
    ...
    def post_to_account(self):
        total_payable_amount = flt(self.total_net_payable_amount, 2)
        accounts = []
        
        # Use the bank account from the document or fallback to company default
        bank_account = self.bank_account or frappe.db.get_value("Company", self.company, "default_bank_account")
        if not bank_account:
    ...
    def get_mr_employee(self):
        ...
        for e in frappe.db.sql(
            ...
            """.format(
                ...
                cond,
                mr_cond,
                frappe.db.escape(self.month),
                frappe.db.escape(self.fiscal_year)
            ),
            as_dict=True,
        ):
            self.append("items", e)
    ...
    def create_mr_invoice(self):
        ...
        for item in self.items:
            ...
            try:
                ...
                net_payable = flt(flt(mr_invoice.grand_total) - salary_tax, 2)
