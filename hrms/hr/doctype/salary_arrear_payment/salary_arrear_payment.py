...
class SalaryArrearPayment(Document):
	...
	def get_employees(self):
		...
		query = """select ss.name as salary_slip, e.name as employee, e.employee_name, e.branch, (select salary_structure from `tabSalary Slip Item` ssi where ssi.parent = ss.name) as salary_struct,
				...
				ifnull((
					select sd.amount from `tabSalary Detail` sd where sd.parent = ss.name
					and sd.salary_component = 'HRA'
				),0) as hra,
				ifnull((
					select sd.amount from `tabSalary Detail` sd where sd.parent = ss.name
					and sd.salary_component = 'Factory Allowance'
				),0) as factory_allowance,
				ifnull((
					select sd.amount from `tabSalary Detail` sd where sd.parent = ss.name
					and sd.salary_component = 'Fuel Allowance'
				),0) as fuel_allowance,
				ifnull((
					select sd.amount from `tabSalary Detail` sd where sd.parent = ss.name
					and sd.salary_component = 'Deployment Allowance'
				),0) as deployment_allowance,
				ifnull((
					select sd.amount from `tabSalary Detail` sd where sd.parent = ss.name
					and sd.salary_component = 'Communication Allowance'
				),0) as communication_allowance,
				ifnull((
					select sd.amount from `tabSalary Detail` sd where sd.parent = ss.name
		...
		for d in entries:
			...
			if d.employee in ("NHDCL0601005","NHDCL2306006","NHDCL2110006","NHDCL8707003","NHDCL2207002","NHDCL1604038"):
				if d.employee =="NHDCL0601005":
					d.contract_allowance = 22082

				if d.employee =="NHDCL2306006":
					d.contract_allowance = 26124

				if d.employee =="NHDCL2110006":
					d.contract_allowance = 19262
			...
			if d.employee in ("NHDCL1805112","NHDCL2104001","NHDCL1904032","NHDCL1805160"):
			...
			if d.employee in ("dsaddad","adsada"):
				...
				d.salary_tax = get_salary_tax((d.basic_pay+d.fixed_allowance))
				d.arrear_pf = 0
				d.arrear_salary_tax = get_salary_tax(d.new_gross_pay)
				d.arrear_employer_pf = 0
