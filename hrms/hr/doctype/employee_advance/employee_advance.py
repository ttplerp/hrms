...
from frappe.utils import flt, nowdate, today, cint, month_diff, get_year_start
from frappe.utils import add_months, get_year_ending, get_first_day, ceil
import erpnext
...
class EmployeeAdvance(Document):
    ...
    def validate_employment_status(self):
        if self.advance_type == "Salary Advance":
            ...
            if employment_type == "Probation":
                frappe.throw(
                    "Employee {}({}) who is in Probation Period is not eligible for Salary Advance."
                )
    ...
    def check_duplicate_advance(self):
        if (
            ...
            frappe.throw(
                "Advance for Travel Request '{}' is already created".format(self.name)
            )
        ...
        if cancel:
            ...
            if self.salary_structure:
                ...
                for d in doc.get("deductions"):
                    if d.salary_component == self.salary_component and self.name in (
                        d.reference_number,
                        d.ref_docname,
                    ):
                        rem_list.append(d)
    ...
    def set_pay_details(self):
        ...
        self.max_months_limit = frappe.get_value(
    ...
    def update_claimed_amount(self, cancel=0):
        claimed_amount = (
            frappe.db.sql(
                """
					...
					FROM `tabExpense Claim Advance` eca, 
						`tabExpense Claim` ec
					WHERE eca.employee_advance = %s
        ...
        if cint(cancel) == 0:
            if flt(self.paid_amount) == flt(claimed_amount):
                frappe.db.set_value(
                    "Employee Advance", self.name, "expenses_claimed", 1
                )
        else:
    ...
    def make_bank_entry(self):
        ...
        ) = get_advance_amount_advance_exchange_rate(advance_account_currency, doc)
...
def calculate_recovery_end_date(start_date, months):
    ...
    # Format the end date as a string in the desired format
