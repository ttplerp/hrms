...
# --- END OF FISCAL YEAR END FUNCTION ---
...
class EmployeeAdvance(Document):
    ...
    def validate_advance_amount(self):
        """Validate advance amount and set recovery dates"""
        # Check if recovery_end_date is manually set
        recovery_end_date_manual = self.get('recovery_end_date')
        
        # Only auto-calculate if recovery_end_date is not set manually
        if not recovery_end_date_manual:
            self.recovery_start_date = get_first_day(today())
            self.recovery_end_date = get_fiscal_year_end_date(today())
        else:
            # Keep the manually set date
            self.recovery_start_date = get_first_day(today())
            # recovery_end_date remains unchanged
        ...
        if flt(self.advance_amount) <= 0:
            ...
            self.db_set("deduction_month", self.deduction_month)

    @frappe.whitelist()
    def validate_deduction_month(self):
        """Validate deduction month and keep recovery_end_date if manually set"""
        if self.advance_type == "Salary Advance":
            # Check if recovery_end_date is manually set
            recovery_end_date_manual = self.get('recovery_end_date')
            ...
            if not recovery_end_date_manual:
                self.recovery_start_date = get_first_day(today())
                self.recovery_end_date = get_fiscal_year_end_date(today())
            # else keep the manually set date
    ...
    def set_pay_details(self):
        """Set pay details and recovery dates"""
    ...
    # --- ADD THESE METHODS FOR BETTER MANUAL DATE HANDLING ---
    
    def on_update(self):
        """Check if recovery_end_date was manually changed"""
        if self.has_value_changed('recovery_end_date'):
            # Flag that recovery_end_date was manually set
            self._recovery_end_date_manual = True
    
    def get(self, fieldname):
        """Override get to check for manual date flag"""
        if fieldname == 'recovery_end_date':
            # If recovery_end_date was manually set, return the stored value
            if getattr(self, '_recovery_end_date_manual', False):
