...
class OvertimeApplication(Document):
	...
	def calculate_totals(self):
		...
		for i in self.get("items"):
			...
			i.is_holiday = holiday_bonus = night_shift_bonus = 0
			if frappe.db.exists("Holiday", {"parent": holiday_list, "holiday_date": str(i.date)}):
				i.is_holiday = 1
				holiday_bonus = 0.5
			if i.is_night_shift:
			...
			i.amount = (flt(i.rate)*(ot_base+holiday_bonus+night_shift_bonus)) * flt(i.number_of_hours)
	...
	def validate_dates(self):				
		for a in self.items:
			...
			elif getdate(a.date) > getdate(today()):
			...
			for i in frappe.db.sql(""" select oa.name from `tabOvertime Application` oa, `tabOvertime Application Item` oai 
						...
						and %(from_date)s <= oai.date
					""", {"employee": self.employee, "name": self.name, "from_date": a.date}, as_dict=True):
